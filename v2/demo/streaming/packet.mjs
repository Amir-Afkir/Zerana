...
export function packetCacheKey(job) {
  ...
  return `stream-packet-v2-bvh/WGS84-ECEF/terrain-v1/${job.source}/${job.profile}/` +
    `${job.source === 'mapbox' ? 'unresolved-preview' : 'ellipsoidal'}/${job.engineering?'real-engineering-v1/':''}${streamCellKey(job.id)}/N${job.subdivisions}`;
}
...
export function validatePacket(bundle, job) {
  ...
  } else if(bundle.engineering||bundle.roadSurface)throw new Error('ROAD_ENGINEERING_UNEXPECTED_PACKET');
  if (bundle.texture) {
...
export async function packetDigest(bundle) {
  ...
  const header = new TextEncoder().encode(JSON.stringify({ id:p.id, source:p.sourceId,
    ...
    engineering:bundle.engineering||null,
